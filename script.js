...
function selectPriorite(cell, categorie, priority) {
    ...
    if (previousSelectedCell) {
        ...
        console.log(`Désélection de ${categorie} : ${previousSelectedCell.textContent}`);
    }
    ...
    prioritesSelectionnees[categorie] = `${categories[categorie]} : ${cell.textContent} (${priority})`;
    ...
    if (selectedMetatypeButton) {
        ...
        console.log(`Désélection du métatype : ${selectedMetatypeButton.textContent}`);
    ...
    if (categorie === "magie_ou_resonnance") generateSpecialButtons(prioriteActuelle); // Utiliser la priorité actuelle
}
...
function generateMetatypeButtons(priorite) {
    ...
    metatypes.forEach((metatype) => {
        ...
        if (selectedMetatype && button.textContent === selectedMetatype.textContent) {
...
function getMetatypesForPriority(priorite) {
    ...
    if (priorite === "C" || priorite === "D" || priorite === "E") return ["Nain", "Elfe", "Humain", "Ork", "Troll"];
    return [];
}
...
function generateSpecialButtons(priorite) {
    ...
    if (priorite === "E") {
        ...
        const specials = ["Adepte", "Adepte mystique", "Magicien pur", "Magicien spécialisé", "Technomancien"]
        ...
        specials.forEach((special) => {
            ...
            // Si le métatype correspond à l'option précédemment sélectionnée, rétablissez la sélection
            if (selectedSpecial && button.textContent === selectedSpecial.textContent) {
                button.classList.add('selected');
...
function handleMetatypeQualities(metatype) {
    if (metatype === "Nain") return ["Résistance aux toxines", "Vision thermographique"];
    if (metatype === "Elfe") return ["Vision nocturne"];
    if (metatype === "Ork") return ["Vision nocturne", "Robuste 1"];
    if (metatype === "Troll") return ["Dépôts dermiques", "Vision thermographique", "Robuste 2"];
    return [];
}
...
function updateAttributsForSpecial(special, priorite) {
    if (priorite === "A") {
        if (special === "Magicien pur" || special === "Adepte mystique" || special === "Adepte") {
...
function handleAttributs() {
    ...
    attributs.forEach(function (attribut) {
        var capitalizedId = attribut.id.charAt(0).toUpperCase() + attribut.id.slice(1);
        attributHTML += `
            <tr>
...
function updateAttributsDisplay() {
    for (const attribut in attributsData) {
        let attribut_base = document.getElementById(attribut + "_base").querySelector("span");
        let attribut_actuel = document.getElementById(attribut + "_actuel").querySelector("span");
        let attribut_max = document.getElementById(attribut + "_max").querySelector("span");
        if (attributsData[attribut].base > attributsData[attribut].value) attributsData[attribut].value = attributsData[attribut].base;
...
function showDepenseAttribut() {
    console.log("showDepenseAttribut - selectedAttributType 1 :", selectedAttributType);

    let nbAttributsDepensePrio = getAttributsDepensePrio(IDselectedCells.attributs);
    let nbAttributsDepenseAjustement = getAttributsDepenseAjuste(IDselectedCells.metatypes);
    ...
    depenseAttributs.innerHTML = ' <table class="table"><thead> <tr> <th scope="col"></th> <th scope="col">Attributs</th> <th scope="col">Ajustement</th></tr> </thead><tbody> <tr> <th scope="row">Points à dépenser</th> <td id="CellAttributsDepensePrio" class="selectable selected" onclick="selectAttributType(this, \'Prio\')"><span id="nbAttributsDepensePrio">' + nbAttributsDepensePrio + '</span></td> <td id="CellAttributsDepenseAjustement" class="selectable" onclick="selectAttributType(this, \'Ajustement\')"><span id="nbAttributsDepenseAjustement">' + nbAttributsDepenseAjustement + '</span></td> </tr></tbody></table>';
}
...
function selectAttributType(cell, type) {
    ...
    const classAttributAjustement = document.getElementById(`CellAttributsDepenseAjustement`);
    if (type === "Prio") {
        classAttributPrio.classList.add("selected");
        if (classAttributAjustement.classList.contains("selected")) classAttributAjustement.classList.remove("selected");
    }
    if (type === "Ajustement") {
        classAttributAjustement.classList.add("selected");
        if (classAttributPrio.classList.contains("selected")) classAttributPrio.classList.remove("selected");
    }
...
function decrementAttribut(attribut, decrement) {
    ...
    if (currentValue >= decrement) {
        ...
        // Mettre à jour le nombre de points d'attributs dépensés
        updateAttributsPoints("decrement", decrement, selectedAttributType, attribut);
    }
}
...
function updateAttributsPoints(type, valeur, selectedAttributType, attribut) {
    ...
    const depenseAttributs = document.getElementById(`nbAttributsDepense${selectedAttributType}`);

    // Mettez à jour le texte avec la nouvelle valeur
    if (selectedAttributType === 'Prio') {     
        depenseAttributs.textContent = nbAttributsDepense + pointsAttributsDepensesPrio;
    } else if (selectedAttributType === 'Ajustement') {     
        depenseAttributs.textContent = nbAttributsDepense + pointsAttributsDepensesAjustement;
    }
    ...
    var cellDepenseAjustement = document.getElementById(`CellAttributsDepenseAjustement`);
    
    console.log("nbAttributsDepense Prio : ", nbAttributsDepense, "pointsAttributsDepensesPrio : ", pointsAttributsDepensesPrio, "Calcul : ", nbAttributsDepense + pointsAttributsDepensesPrio)
...
function getAttributsDepensePrio(priorite) {
    ...
    else console.log("function getAttributsDepensePrio(priorite) ne contient pas la bonne propriété en paramètre : " + priorite);
}
...
function getAttributsDepenseAjuste(priorite) {
    ...
    else console.log("function getAttributsDepenseAjuste(priorite) ne contient pas la bonne propriété en paramètre : " + priorite);
}
...
function afficherResultats() {
    ...
    const metatypeForm = document.getElementById("metatypeButtons");
    const selectedMeta = metatypeForm.querySelector('button.selected') || selectedMetatype;
    if (selectedMeta) {
        const selectedMetatypeValue = selectedMeta.textContent || selectedMetatype;
        resultat.innerHTML += "<h5>Métatype sélectionné :</h5>" + selectedMetatypeValue;
        const metatypeQualities = handleMetatypeQualities(selectedMetatypeValue);
        if (metatypeQualities.length > 0) resultat.innerHTML += "<h5>Traits innés :</h5>" + metatypeQualities.join(', ');
    }

    const specialForm = document.getElementById("specialButtons");
    const selectedSpe = specialForm.querySelector('button.selected') || selectedSpecial;
    if (selectedSpe) {
...
resetButton.addEventListener("click", function () {
    ...
    const userConfirmed = confirm("Êtes-vous sûr de vouloir réinitialiser la sélection ?");

    if (userConfirmed) {
        // Effacez le cookie en définissant une date d'expiration passée
        const pastExpirationDate = new Date(0);
        document.cookie = "selectionData=; expires=" + pastExpirationDate.toUTCString() + "; path=/";
